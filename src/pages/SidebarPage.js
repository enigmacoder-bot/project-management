                ...
                <div className="h-full flex flex-col justify-between">
                        ...
                        <ul className="space-y-2 font-medium">
                            {SIDEBAR_ITEMS.map((item) => (
                                <li key={item.href}>
                                    <Link
                                        to={item.href}
                                        className={`flex items-center p-2 pl-4 rounded-lg group ${location.pathname === item.href
                                            ? "bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white"
                                            : "text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700"
                                            }`}
                                    >
                                        <item.icon className={`w-6 h-6 transition duration-75 ${location.pathname === item.href
                    ...
                    <div className="flex justify-end p-4">
                        <button
                            ...
                            className={`w-10 h-10 rounded-full border-2 ${language === "en" ? "border-blue-500" : "border-transparent"} overflow-hidden`}
                        >
                            <img src={usEnglishIcon} alt="English" className="w-full h-full object-cover" />
                        </button>
                        <button
                            onClick={() => handleLanguageChange("ar")}
                            className={`w-10 h-10 rounded-full border-2 ml-2 ${language === "ar" ? "border-blue-500" : "border-transparent"} overflow-hidden`}
                        >
                            <img src={arabicIcon} alt="Arabic" className="w-full h-full object-cover" />
                        </button>
                    </div>
                </div>
            </div>
